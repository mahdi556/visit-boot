...
export default function StoreFormModal({ 
  ...
  // اضافه کردن useEffect برای آپدیت formData وقتی editingStore تغییر می‌کند
  useEffect(() => {
    if (editingStore) {
      setFormData({
  ...
  }, [editingStore, open]); // وابستگی به editingStore و open
                ...
                >
                  {selectedLocation ? (
                    <Box>
                      ...
                      <Typography color="success.main" fontWeight="600" gutterBottom>
                        موقعیت مشخص شده
                      </Typography>
                      <Chip 
